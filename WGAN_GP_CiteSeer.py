...
def train(generator, discriminator, train_loader, n_critic, lambda_gp, epochs, device, dataset_stats, num_classes):
    ...
    # Save models
    os.makedirs('./saved_models/CiteSeer_WGANGP', exist_ok=True)
    torch.save(generator.state_dict(), './saved_models/CiteSeer_WGANGP/generator.pt')
    torch.save(discriminator.state_dict(), './saved_models/CiteSeer_WGANGP/discriminator.pt')
    print("Models saved.")
...
def evaluate(generator, train_loader, test_loader, device, dataset_stats, num_classes):
    ...
    model_path = "./saved_models/CiteSeer_WGANGP/generator.pt"
...
# ---------------------------------------------------
# 2) Helper: make a graph-level label from node labels
# ---------------------------------------------------
def majority_class_label(node_y: torch.Tensor) -> int:
    """
    Returns the most frequent class id in node_y.
    Tie-breaker: pick the smallest class id among ties.
    """
    # Ensure 1D CPU list of ints
    vals = node_y.view(-1).tolist()
    counts = Counter(vals)
    max_cnt = max(counts.values())
    # classes achieving the max count
    candidates = [c for c, v in counts.items() if v == max_cnt]
    return int(min(candidates))

# ---------------------------------------------------------
# 3) Build a graph-level dataset from CiteSeer subgraphs
# ---------------------------------------------------------
class CiteSeerSubgraphGraphLabelDataset(InMemoryDataset):
    """
    Wraps a list of PyG Data subgraphs (with node-level labels) and assigns each
    a graph-level target y = majority(node_y). Keeps original node labels in `node_y`.
    """
    def __init__(self, subgraphs, transform=None, pre_transform=None):
        super().__init__('.', transform, pre_transform)
        # Build processed list of Data with scalar y and node_y backup
        data_list = []
        for g in subgraphs:
            g = g.clone()

            # Expect node labels in g.y (Planetoid format). Save a copy.
            assert g.y.dim() == 1 or (g.y.dim() == 2 and g.y.size(1) == 1), \
                "Expected node-level labels in `g.y`"
            node_y = g.y.view(-1).clone()

            # Compute majority class
            graph_y = majority_class_label(node_y)

            # Move node labels aside to `node_y`, set graph-level y as scalar tensor
            g.node_y = node_y
            g.y = torch.tensor([graph_y], dtype=torch.long)

            data_list.append(g)

        self.data, self.slices = self.collate(data_list)

         # --- make num_classes available ---
        y_all = self.data.y.view(-1)              # all graph-level labels
        self._num_classes = int(torch.unique(y_all).numel())

         # ---- cache num_features ----
        # Prefer reading from an actual item (handles ragged storage & safety).
        if len(subgraphs) > 0 and hasattr(subgraphs[0], "x") and subgraphs[0].x is not None:
            self._num_features = int(subgraphs[0].x.size(-1))
        elif hasattr(self.data, "x") and self.data.x is not None:
            self._num_features = int(self.data.x.size(-1))
        else:
            self._num_features = 0  # fall back if no features provided
...
# -----------------------------
# 4) End-to-end data preparation
# -----------------------------
def make_citeseer_graphlevel_dataset(num_parts=200, seed=42):
    torch.manual_seed(seed)

    # Load single-graph CiteSeer
    dataset = Planetoid(
        root="data/CiteSeer",
        name="CiteSeer",
        transform=T.Compose([ToUndirected(), T.NormalizeFeatures()])
    )
    full = dataset[0]

    # Partition into subgraphs
    cluster_data = ClusterData(full, num_parts=num_parts, recursive=False)
    loader = ClusterLoader(cluster_data, batch_size=1, shuffle=True)
    subgraphs = [batch.clone() for batch in loader]  # each is a Data (induced subgraph)

    # Wrap with graph-level labels
    graph_dataset = CiteSeerSubgraphGraphLabelDataset(subgraphs)
...
def main():
    ...
    # dataset = Planetoid(root='./data/CiteSeer', name='CiteSeer').shuffle()
    # data = dataset[0]
    NUM_PARTS = 200
    graph_ds = make_citeseer_graphlevel_dataset(num_parts=NUM_PARTS, seed=42)


    # Analyze dataset statistics before splitting
    print("Analyzing dataset statistics...")

    dataset_stats = analyze_dataset_statistics(graph_ds)
    dataset_stats = normalize_stats(dataset_stats)
    
    num_classes = graph_ds.num_classes
    
    split_idx = int(0.85 * NUM_PARTS)
    train_dataset= graph_ds[:split_idx]
    test_dataset  = graph_ds[split_idx:]
    ...
    num_node_features = graph_ds.num_node_features
    ...
    print(f"\nDataset info:")
    print(f"Total graphs: {len(graph_ds)}")
